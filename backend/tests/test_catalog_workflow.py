...
on the EquipmentItem catalog. ``test_catalog_api.py`` keeps the M3
...
async def _clean(async_session_factory: async_sessionmaker[AsyncSession]):
    ...
    async with async_session_factory() as session, session.begin():
        ...
        await session.execute(text("DELETE FROM equipment_item WHERE display_name LIKE 'M7-%'"))


# --- helpers --------------------------------------------------------------


async def _propose_equipment_item(
    client: AsyncClient,
    csrf: str,
    *,
    display_name: str,
    ...
    # The EquipmentItem identity tuple is (category, brand, model).
    # Default the model to the display_name so two bare-bones proposals
    # don't collide on (tools, NULL, NULL).
    ...
    if brand is not None:
        body["brand"] = brand
    resp = await post_with_csrf(client, csrf, "/api/equipment-items", json=body)
...
async def test_admin_create_equipment_item_directly_approved(
    client: AsyncClient,
    ...
    resp = await post_with_csrf(
        ...
        csrf_admin,
        "/api/equipment-items",
        json={"category": "tools", "display_name": "M7-AdminRT-Approved"},
    )
    assert resp.status_code == 201, resp.text
    body = resp.json()
...
async def test_admin_rejects_with_reason(
    ...
    entry_id = await _propose_equipment_item(client, csrf_editor, display_name="M7-Reject-Cand")
    ...
    resp = await post_with_csrf(
        client,
        csrf_admin,
        f"/api/equipment-items/{entry_id}/reject",
...
async def test_admin_rejects_requires_non_empty_reason(
    ...
    _, csrf_editor = await login_as(client, async_session_factory, role=UserRole.EDITOR)
    entry_id = await _propose_equipment_item(client, csrf_editor, display_name="M7-Reject-NoReason")

    await client.post("/api/auth/logout")
    _, csrf_admin = await login_as(client, async_session_factory, role=UserRole.ADMIN)
    resp = await post_with_csrf(
        client, csrf_admin, f"/api/equipment-items/{entry_id}/reject", json={"reason": ""}
    )
...
async def test_editor_cannot_reject(
    ...
    entry_id = await _propose_equipment_item(client, csrf, display_name="M7-Reject-Editor")
    resp = await post_with_csrf(
        client, csrf, f"/api/equipment-items/{entry_id}/reject", json={"reason": "x"}
    )
...
async def test_cannot_reject_already_approved(
    ...
    _, csrf_editor = await login_as(client, async_session_factory, role=UserRole.EDITOR)
    entry_id = await _propose_equipment_item(client, csrf_editor, display_name="M7-Reject-Approved")
    await client.post("/api/auth/logout")
    _, csrf_admin = await login_as(client, async_session_factory, role=UserRole.ADMIN)
    appr = await post_with_csrf(client, csrf_admin, f"/api/equipment-items/{entry_id}/approve")
    assert appr.status_code == 200
    resp = await post_with_csrf(
        client, csrf_admin, f"/api/equipment-items/{entry_id}/reject", json={"reason": "late"}
    )
    assert resp.status_code == 409, resp.text


async def test_admin_rejects_equipment_item(
    client: AsyncClient,
    async_session_factory: async_sessionmaker[AsyncSession],
) -> None:
    _, csrf_editor = await login_as(client, async_session_factory, role=UserRole.EDITOR)
    rt_id = await _propose_equipment_item(client, csrf_editor, display_name="M7-RT-RejectMe")
    ...
    resp = await post_with_csrf(
        client,
        csrf_admin,
        f"/api/equipment-items/{rt_id}/reject",
...
async def test_editor_withdraws_own_pending(
    ...
    _, csrf = await login_as(client, async_session_factory, role=UserRole.EDITOR)
    entry_id = await _propose_equipment_item(client, csrf, display_name="M7-Withdraw-Own")
    resp = await delete_with_csrf(client, csrf, f"/api/equipment-items/{entry_id}")
    assert resp.status_code == 204, resp.text
    listing = await client.get("/api/equipment-items")
...
async def test_editor_cannot_withdraw_foreign_pending(
    ...
    entry_id = await _propose_equipment_item(client, csrf_a, display_name="M7-Withdraw-Foreign")
    await client.post("/api/auth/logout")
    ...
    # their RLS view → 404.
    resp = await delete_with_csrf(client, csrf_b, f"/api/equipment-items/{entry_id}")
...
async def test_editor_cannot_withdraw_own_rejected(
    ...
    entry_id = await _propose_equipment_item(
    ...
    csrf_editor = await login(client, email, password)
    resp = await delete_with_csrf(client, csrf_editor, f"/api/equipment-items/{entry_id}")
    assert resp.status_code == 409, resp.text
...
async def test_admin_can_withdraw_any_pending(
    ...
    await client.post("/api/auth/logout")
    _, csrf_admin = await login_as(client, async_session_factory, role=UserRole.ADMIN)
    resp = await delete_with_csrf(client, csrf_admin, f"/api/equipment-items/{entry_id}")
    assert resp.status_code == 204, resp.text
...
async def test_admin_updates_equipment_item_all_fields(
    client: AsyncClient,
    async_session_factory: async_sessionmaker[AsyncSession],
) -> None:
    _, csrf_editor = await login_as(client, async_session_factory, role=UserRole.EDITOR)
    rt_id = await _propose_equipment_item(
        client, csrf_editor, display_name="M7-RT-Update-Old", category="tools"
    )
    await client.post("/api/auth/logout")
    _, csrf_admin = await login_as(client, async_session_factory, role=UserRole.ADMIN)
...
async def test_admin_update_unique_conflict_returns_409(
    ...
    """EquipmentItem identity is (category, brand, model)."""
    _, csrf_editor = await login_as(client, async_session_factory, role=UserRole.EDITOR)
    first_id = await _propose_equipment_item(
        client, csrf_editor, display_name="M7-Conflict-First", brand="ACME", model="One"
    )
    second_id = await _propose_equipment_item(
    ...
    resp = await patch_with_csrf(
        client,
        csrf_admin,
        f"/api/equipment-items/{second_id}",
        json={"model": "One"},
    )
    assert resp.status_code == 409, resp.text
...
async def test_editor_cannot_patch(
    ...
    _, csrf_editor = await login_as(client, async_session_factory, role=UserRole.EDITOR)
    entry_id = await _propose_equipment_item(client, csrf_editor, display_name="M7-Editor-Patch")
    resp = await patch_with_csrf(
        client,
        csrf_editor,
        f"/api/equipment-items/{entry_id}",
...
async def test_listing_status_filter(
    ...
    _, csrf_editor = await login_as(client, async_session_factory, role=UserRole.EDITOR)
    pending_id = await _propose_equipment_item(
        client, csrf_editor, display_name="M7-Filter-Pending"
    )
    approved_id = await _propose_equipment_item(
        client, csrf_editor, display_name="M7-Filter-Approved"
    )
    rejected_id = await _propose_equipment_item(
        client, csrf_editor, display_name="M7-Filter-Rejected"
    )
    await client.post("/api/auth/logout")

    _, csrf_admin = await login_as(client, async_session_factory, role=UserRole.ADMIN)
    appr = await post_with_csrf(client, csrf_admin, f"/api/equipment-items/{approved_id}/approve")
    assert appr.status_code == 200
    rej = await post_with_csrf(
        client,
        csrf_admin,
        f"/api/equipment-items/{rejected_id}/reject",
    ...
    for status_value, expected in (
        ...
        resp = await client.get(f"/api/equipment-items?status={status_value}")
        assert resp.status_code == 200, resp.text
...
async def test_editor_sees_own_rejected_in_listing(
    ...
    _, email, password = await make_user(async_session_factory, role=UserRole.EDITOR)
    csrf_editor = await login(client, email, password)
    entry_id = await _propose_equipment_item(client, csrf_editor, display_name="M7-OwnRejectedVis")
    await client.post("/api/auth/logout")
    ...
    csrf_editor = await login(client, email, password)
    resp = await client.get("/api/equipment-items?status=rejected")
...
async def test_other_editor_cannot_see_foreign_rejected(
    ...
    entry_id = await _propose_equipment_item(client, csrf_a, display_name="M7-ForeignRejected")
    await client.post("/api/auth/logout")
    _, csrf_admin = await login_as(client, async_session_factory, role=UserRole.ADMIN)
    rej = await post_with_csrf(
        client,
